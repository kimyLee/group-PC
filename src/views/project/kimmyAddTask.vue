<template>
  <!--在最外层定一个单独的类，比如myAdd， 该组件样式都包在它里面，这个组件的样式就不会和其他组件冲突-->
  <div class="myAdd">
  <div v-if="!isEdit" class="add-task" @click="isEdit=true">
    <i class="fa fa-plus-circle" aria-hidden="true"></i>
    我抄袭你的添加任务
  </div>
  <!--添加任务模块-->
  <form v-if="isEdit" action="" class="task-editor">
    <!--不知道这个bind还需不需要-->
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        // todo: vue 是数据驱动的，要用v-show he if 来决定显示问题
        isEdit: false,
        tasktitle: '',
        taskmember: '',
        taskpriority: 0,
